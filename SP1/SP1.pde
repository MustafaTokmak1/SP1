...
void draw() {
  ...
  if (game.getPlayerLife() > 0 && game.getPlayer2Life() > 0) {
    fill(255);
    text("Player 1 lifes: "+game.getPlayerLife(), 25, 25);
    text ("Player 2 lifes: "+game.getPlayer2Life(), width-200, 25);
  } else if (game.getPlayerLife() <= 0) {
    fill(255);
    textSize(80);
    text("Player 2 wins ", width/2-230, height/2);
    noLoop();
  } else if (game.getPlayer2Life() <= 0) {
    fill(255);
    textSize(80);
    text("Player 1 wins ", width/2-230, height/2);
    noLoop();
  }
}
